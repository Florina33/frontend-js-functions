...
/******/ 	var hotCurrentHash = "6db6e0670080e35784df"; // eslint-disable-line no-unused-vars
...
function min(arrNumbers = []) {
	...
	return arrNumbers.filter(isNumber).reduce(getLess, undefined);
}
...
function max(arrNumbers = []) {
	...
	return arrNumbers.filter(isNumber).reduce(getGreater, undefined);
}
...
function sum(...arrNumbers) {
	...
	return arrNumbers.filter(isNumber).reduce(getSum, 0);
}
